...
\index{Semicolons}
A semicolon \mintinline{cpp}{;} is placed after each instruction in \Cpp{}. This semicolon tells the compiler that an instruction ends there. If you forget to place a semicolon, the program is not correct and you will not be able to compile the program. The compiler will then give an error message.

\subsection{Braces}
\index{Braces}
Braces \mintinline{cpp}{{}} are used to indicate a block of code. The braces must be balanced in the code, which means that for every opening brace \texttt{\{} there must be a corresponding closing brace \texttt{\}}. The use of braces is later illustrated with the control structures (if-then-else, for, ..), functions and classes.

\subsection{Comments}
% comments in C++
% // of /* */
To increase the readability of your code, it is useful to add comments. In this comment\index{Comment} you describe what this part of the code does. This makes it clear what you have programmed correctly, even if you view your code again later.\\

\noindent In \Cpp{} there are two different ways to add comments.\\
Comments on one line are indicated by \mintinline{cpp}{//}. All text after \mintinline{cpp}{//} up to the end of the line is considered a comment and will therefore not be executed.
...
\begin{example}[Declare an \emph{integer} variable and assign a value]
	\phantom{ }
	\begin{minted}{cpp}
/*
...
\begin{example}[Declare and initialize different data types]
	\phantom{ }
	\begin{minted}{cpp}
bool b = true;
char c = 'q';
...
To adjust the value in the variables, we use \emph{operators}. Below we provide an overview of the most important operators in \Cpp.
...
\begin{center}
	\begin{tabular}{c l l}
		...
		\textbf{Operator} & \textbf{Description}                     & \textbf{Example}                                              \\ \midrule
		   ...
		   \texttt{++}    & Increase the value of a variable by 1    & \mintinline{cpp}{A++} $\Rightarrow$ \texttt{A} $\rightarrow 11$ \\
		  \texttt{-{}-}   & Decrease the value of a variable by 1    & \mintinline{cpp}{A--} $\Rightarrow$ \texttt{A} $\rightarrow 9$  \\ \bottomrule
	\end{tabular}
\end{center}
...
\begin{center}
	\begin{tabular}{c l l}
		\toprule
		\textbf{Operator} & \textbf{Description}     & \textbf{Example}                                     \\ \midrule
		   \texttt{==}    & Equality                  & \mintinline{cpp}{10 == 3} $\rightarrow$ \texttt{false} \\
		   \texttt{!=}    & Not equal                 & \mintinline{cpp}{10 != 3} $\rightarrow$ \texttt{true}  \\
		   \texttt{>}     & Greather than             & \mintinline{cpp}{10 > 10} $\rightarrow$ \texttt{false} \\
